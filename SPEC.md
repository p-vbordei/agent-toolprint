...
```
{
  ...
  "call": {
    ...
    "args_enc": "jcs" | "cbor"        // which canonicalization was hashed
  },
  "result": {
    "status": "ok" | "error",
    "response_hash": "sha256:...",    // over JCS(response)
    "bytes?": "<base64 inline body, <= 1 KiB>"
  },
...
Canonical encoding: JCS ([RFC 8785](https://www.rfc-editor.org/rfc/rfc8785)) by default; deterministic CBOR ([RFC 8949 §4.2](https://datatracker.ietf.org/doc/html/rfc8949)) opt-in.
...
- **Replay**: `nonce` + `ts` together SHOULD be used to detect replays.
- **Args / response privacy**: fields can be hash-only; plaintext bodies are optional and size-capped.
- **Key rotation**: follows `agent-id` rules. Verifiers MUST use the DID Document state at `ts`, not "now".
